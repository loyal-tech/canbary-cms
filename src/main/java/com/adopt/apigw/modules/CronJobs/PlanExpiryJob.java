...
//    }
    ...
    @Scheduled(cron = "${cronJobTimeForPlanExpiryStatusJob}")
    public void cronJobPlanExpiryStatus() throws Exception {
        log.info("XXXXXXXXXXXX----------Plan Expiry Status Scheduler START---------XXXXXXXXXXXX");
        SchedulerAudit schedulerAudit = new SchedulerAudit();
        schedulerAudit.setStartTime(LocalDateTime.now());
        schedulerAudit.setSchedulerName(CronjobConst.SCHEDULER_AUDIT.PLAN_STATUS_EXPIRY_JOB_SCHEDULER);
        if (!schedulerLockService.isSchedulerLocked(CronjobConst.PLAN_STATUS_EXPIRY_JOB)) {
            schedulerLockService.acquireSchedulerLock(CronjobConst.PLAN_STATUS_EXPIRY_JOB);
            try {
                int updatedCount = postpaidPlanRepo.markPlansAsExpired(LocalDate.now(), "Expired",LocalDateTime.now());
                schedulerAudit.setEndTime(LocalDateTime.now());
                schedulerAudit.setDescription("Plan Status Expiry Scheduler Success");
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_SUCCESS);
                schedulerAudit.setTotalCount(updatedCount);
                log.info("Total plans expired: {}", updatedCount);
            } catch (Exception ex) {
                schedulerAudit.setEndTime(LocalDateTime.now());
                schedulerAudit.setDescription(ex.getMessage());
                schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_FAILURE);
                ...
                schedulerAuditService.saveEntity(schedulerAudit);
                schedulerLockService.releaseSchedulerLock(CronjobConst.PLAN_STATUS_EXPIRY_JOB);
                log.info("XXXXXXXXXXXX---------- Plan Status Expiry Scheduler Locked released ---------XXXXXXXXXXXX");
            }
        } else {
            schedulerAudit.setEndTime(LocalDateTime.now());
            schedulerAudit.setDescription("Plan Status Expiry Scheduler Lock held by another instance");
            schedulerAudit.setStatus(CronjobConst.SCHEDULER_AUDIT.SCHEDULER_STATUS_LOCKED);
            schedulerAuditService.saveEntity(schedulerAudit);
            log.warn("XXXXXXXXXXXX----------Plan Status Expiry Scheduler Locked held by another instance---------XXXXXXXXXXXX");
        }
    }
